...
const DisplayData = () => {
   ...
   useEffect(() => {
     const fetchData = async () => {
       try {
         const snapshot = await db.collection("data").get().then((querySnapshot) => {console.log(querySnapshot.size)});
  ...
  <div style={{width: 1112, height: 742, position: 'relative'}}>
    ...
    <div key={index}>
        ...
        <div style={{width: 1004, height: 321, left: 58, top: 349, position: 'absolute', background: '#FBF3F3', boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', borderRadius: 35}} />
          ...
          <div style={{left: 88, top: 391, position: 'absolute', color: 'black', fontSize: 16, fontFamily: 'Inter', fontWeight: '400', wordWrap: 'break-word'}}>Blood Group:{item.booldgroup}y</div>
